...
type CompactStatement = {
  ...
  period: string;
  currency: string;
  transactions: Array<{ date: string; narration: string; merchant: string; amount: number; type: "debit" | "credit"; category: Category; confidence: number }>;
  insights: string[];
};

function parseModelJson(text: string): CompactStatement {
  const cleaned = text.replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "").trim();
  try { return JSON.parse(cleaned) as CompactStatement; }
  catch { throw new Error("The statement extraction was incomplete. Please retry the file; if it is unusually large, export it as CSV or split the PDF by date range."); }
}
...
function canonicalStatement(parsed: CompactStatement, filename: string): StatementResult {
  const transactions = parsed.transactions.flatMap<Transaction>((row) => {
    ...
    return [{
      id: crypto.randomUUID(), date: String(row.date), merchant: String(row.merchant || normalizeMerchant(narration)).trim() || normalizeMerchant(narration),
      description: narration, amount, type: row.type === "credit" ? "credit" : "debit", category: row.category,
      confidence: Math.max(0, Math.min(1, Number(row.confidence) || 0)), source: filename, explanation: "Categorized from the statement narration.",
    }];
  });
  if (!transactions.length) throw new Error("No transactions could be read from this statement.");
  return {
    accountName: String(parsed.accountName || "Imported account"), bankName: String(parsed.bankName || filename), period: String(parsed.period || "Imported statement"),
    currency: String(parsed.currency || "INR"), transactions, insights: Array.isArray(parsed.insights) ? parsed.insights.slice(0, 3).map(String) : [],
  };
}

export async function processStatementInput(body: StatementInput): Promise<StatementResult & { provider?: string; model?: string }> {
  const filename = body.filename || "statement";
  const localResult = () => body.text ? parseCsvFallback(body.text, filename) : null;
  const providers = configuredProviders();
  if (!providers.vertex && !providers.groq) {
    const parsed = localResult();
    if (parsed) return parsed;
    throw new StatementProcessingError("Document intelligence is not configured. Add Vertex AI credentials to process PDF or image statements.", 503);
  }
  ...
  try {
    ...
    let result;
    try { result = await generateWithFallback(extraction); }
    catch (error) {
      const message = error instanceof Error ? error.message : "";
      if (!/incomplete structured data|output limit/i.test(message)) throw error;
      result = await generateWithFallback({ ...extraction, prompt: `${extraction.prompt}\nThe previous response was cut off. Return one complete, valid JSON object. Keep narration compact and close every string, array, and object.` });
    }
    return { ...canonicalStatement(parseModelJson(result.text), filename), provider: result.provider, model: result.model };
  } catch (error) {
    const parsed = localResult();
    if (parsed) return { ...parsed, provider: "local" };
