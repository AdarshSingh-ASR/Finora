...
const consumptionCategories = new Set(["Food & Dining", "Housing", "Transport", "Shopping", "Bills & Utilities", "EMI", "Health", "Entertainment", "Travel", "Miscellaneous", "Other"]);
...
const merchantName = (item) => String(item.merchant || item.description || "Unknown merchant").trim();
const median = (values) => {
...
function subscriptionRows(transactions) {
  const groups = transactions.filter((item) => item.type === "debit").reduce((acc, item) => {
...
export function buildCashFlow(transactions, requestedPeriod) {
  ...
  const income = sum(items.filter((item) => item.type === "credit"));
