...
async function generateWithVertex({ system, prompt, schema, media, maxOutputTokens }) {
  ...
  const response = await fetch(`https://${host}/v1/projects/${encodeURIComponent(credentials.project_id)}/locations/${encodeURIComponent(location)}/publishers/google/models/${encodeURIComponent(model)}:generateContent`, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: { temperature: 0.1, maxOutputTokens: Math.max(1024, Math.min(Number(maxOutputTokens) || 16384, 65535)), ...(schema ? { responseMimeType: "application/json", responseSchema: schema } : {}) },
    }),
  });
  const payload = await response.json();
  if (!response.ok) throw new Error(`Vertex AI request failed (${response.status}).`);
  const candidate = payload.candidates?.[0];
  if (candidate?.finishReason === "MAX_TOKENS") throw new Error("Vertex AI reached the output limit before finishing the statement.");
  const text = candidate?.content?.parts?.map((part) => part.text || "").join("").trim();
  if (!text) throw new Error("Vertex AI returned no text.");
  if (schema) {
    try { parseStructuredText(text); }
    catch { throw new Error("Vertex AI returned incomplete structured data."); }
  }
...
async function generateWithGroq({ system, prompt, schema }) {
  ...
  if (schema) {
    try { parseStructuredText(text); }
    catch { throw new Error("Groq returned incomplete structured data."); }
  }
