...
import { budgetStatus, buildFinancialTimeline, categories, classifyNarration, compareMonths, detectAnomalies, detectSubscriptions, financialHealthScore, findDuplicateTransactions, inPeriod, latestPeriod, money, normalizeMerchant, refineTransactionsForAnalysis, summarize, weeklyReport } from "../../lib/finance";
...
export default function Home() {
  ...
  const analysisTransactions = useMemo(() => refineTransactionsForAnalysis(statement.transactions), [statement]);
  const activePeriod = useMemo(() => latestPeriod(analysisTransactions), [analysisTransactions]);
  const currentTransactions = useMemo(() => inPeriod(analysisTransactions, activePeriod), [analysisTransactions, activePeriod]);
  ...
  const comparison = useMemo(() => compareMonths(analysisTransactions, activePeriod), [analysisTransactions, activePeriod]);
  const subscriptions = useMemo(() => detectSubscriptions(analysisTransactions), [analysisTransactions]);
  const duplicateMatches = useMemo(() => findDuplicateTransactions(analysisTransactions), [analysisTransactions]);
  const anomalies = useMemo(() => detectAnomalies(analysisTransactions), [analysisTransactions]);
  const budgetStatuses = useMemo(() => budgetStatus(analysisTransactions, budgets, activePeriod), [analysisTransactions, budgets, activePeriod]);
  const health = useMemo(() => financialHealthScore(analysisTransactions, budgets), [analysisTransactions, budgets]);
  const financialTimeline = useMemo(() => buildFinancialTimeline(analysisTransactions, budgets, 6), [analysisTransactions, budgets]);
  const week = useMemo(() => weeklyReport(analysisTransactions), [analysisTransactions]);
  const weeklyReportDebits = useMemo(() => analysisTransactions.filter((transaction) => transaction.type === "debit" && transaction.date.slice(0, 10) >= week.start && transaction.date.slice(0, 10) <= week.end), [analysisTransactions, week.start, week.end]);
  ...
  const weeklyConsumptionDebits = useMemo(() => weeklyReportDebits.filter((transaction) => !["Transfers", "Investment"].includes(transaction.category)), [weeklyReportDebits]);
  const weeklyTopCategory = useMemo(() => Object.entries(weeklyConsumptionDebits.reduce<Record<string, number>>((acc, transaction) => { acc[transaction.category] = (acc[transaction.category] || 0) + transaction.amount; return acc; }, {})).sort((a, b) => b[1] - a[1])[0], [weeklyConsumptionDebits]);
  const weeklyTopMerchant = useMemo(() => Object.entries(weeklyConsumptionDebits.reduce<Record<string, number>>((acc, transaction) => { const merchant = normalizeMerchant(transaction.merchant || transaction.description); acc[merchant] = (acc[merchant] || 0) + transaction.amount; return acc; }, {})).sort((a, b) => b[1] - a[1])[0], [weeklyConsumptionDebits]);
  const weeklyLargest = useMemo(() => [...weeklyConsumptionDebits].sort((a, b) => b.amount - a.amount)[0], [weeklyConsumptionDebits]);
  ...
  const reportConsumptionDebits = useMemo(() => reportDebits.filter((transaction) => !["Transfers", "Investment"].includes(transaction.category)), [reportDebits]);
  const reportTopCategory = useMemo(() => Object.entries(reportConsumptionDebits.reduce<Record<string, number>>((acc, transaction) => { acc[transaction.category] = (acc[transaction.category] || 0) + transaction.amount; return acc; }, {})).sort((a, b) => b[1] - a[1])[0], [reportConsumptionDebits]);
  const reportTopMerchant = useMemo(() => Object.entries(reportConsumptionDebits.reduce<Record<string, number>>((acc, transaction) => { const merchant = normalizeMerchant(transaction.merchant || transaction.description); acc[merchant] = (acc[merchant] || 0) + transaction.amount; return acc; }, {})).sort((a, b) => b[1] - a[1])[0], [reportConsumptionDebits]);
  const reportLargest = useMemo(() => [...reportConsumptionDebits].sort((a, b) => b.amount - a.amount)[0], [reportConsumptionDebits]);
  const monthlyReportSuggestion = useMemo(() => reportTopCategory
    ? `${reportTopCategory[0]} was the largest consumption area at ${money(reportTopCategory[1])}${reportTopMerchant ? `, led by ${reportTopMerchant[0]} at ${money(reportTopMerchant[1])}` : ""}.${subscriptions.length ? ` ${subscriptions.length} recurring expense${subscriptions.length === 1 ? " is" : "s are"} currently detected.` : ""}`
    : "No consumption spending was detected in this period; transfers and investments are reported separately.", [reportTopCategory, reportTopMerchant, subscriptions]);
  ...
  async function runAgentAction(messageId: string, action: AgentAction) {
    ...
    try {
      ...
      if (action.type === "open_sheet") {
        ...
        const description = action.payload.description || action.payload.merchant;
        const classified = classifyNarration({ merchant: action.payload.merchant, description, type: action.payload.direction });
        const transaction: Transaction = { id: crypto.randomUUID(), date: action.payload.date || new Date().toISOString().slice(0, 10), merchant: action.payload.category ? action.payload.merchant : classified.merchant, description, amount: action.payload.amount, type: action.payload.direction, category: (action.payload.category as Category) || classified.category, confidence: action.payload.category ? 1 : classified.confidence, source: "Ask Finora", explanation: action.payload.category ? "Added and confirmed by you through Ask Finora." : classified.reason };
        if (attachmentOnly) {
          let added = false;
